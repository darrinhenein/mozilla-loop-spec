...
var NewCallView = React.createClass({displayName: 'NewCallView',
  ...
  onClick: function() {
    if(this.state.callName == '') {
      $(this.refs.callerNameInput.getDOMNode()).addClass('error');
      return;
    }
  ...
  render: function(){
    return (
      React.DOM.div( {className:"SearchBarWrapper"}, 
        React.DOM.input( {className:"SearchBar", value:this.props.val, onChange:this.props.handleChange, placeholder:"Search..."}),React.DOM.i( {className:"fa fa-book"})
      )
    )
  }
});
  ...
  render: function() {
    return (
        BaseState( {name: this.props.name,  index: this.props.index }, 
          ...
          PanelGroup(null, 
            Panel( {items: this.props.items }, 
              Header( {username:"ally@gmail.com", editText:"View Profile"}),
