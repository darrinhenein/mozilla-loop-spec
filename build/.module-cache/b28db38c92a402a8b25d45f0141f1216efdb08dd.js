...
var NewCallView = React.createClass({displayName: 'NewCallView',
  ...
  getTimeFromRange: function(position) {
    ...
    if(dur > 60*24)
    {
      return parseInt(dur.asDays(), 10) + 'days';
    } else {
      return parseInt(dur.asHours(), 10) + 'hours';
    }
    console.log(dur.asHours());
  },
  getInitialState: function(){
    return {
      callDuration: 15
  ...
  render: function(){
    var unit = this.state.callDuration > 1 ? 'days' : 'day';
    return (
      React.DOM.div( {className:"NewCallView"}, 
        ...
        React.DOM.h3(null, React.DOM.i( {className:"fa fa-clock-o"}), " Invitation will be valid for ",  this.getTimeFromRange(this.state.callDuration),  " ",  unit )
      )
    )
  }
});
