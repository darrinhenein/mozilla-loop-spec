    ...
    if(this.state.hasLink) {
      return (
        React.DOM.div( {className:"NewCallView"}, 
          React.DOM.h3(null,  this.state.callName ),
      ...
      return (
        React.DOM.div( {className:"NewCallView"}, 
          ...
          React.DOM.input( {onChange:this.onTextChange, valueLink:this.linkState('callName'), placeholder:"Enter invite name"}),
          React.DOM.h3(null, React.DOM.i( {className:"fa fa-clock-o"}), " Invitation will expire in ",  this.getTimeFromRange(this.state.callDuration) ),
          React.DOM.input( {valueLink:this.linkState('callDuration'), type:"range", name:"time", min:"1", max:"100"}),
          Button( {enabled: this.state.canGetLink,  onClick:this.onClick, style:"action", text:"Get Link"})
        )
      )
    }
  }
