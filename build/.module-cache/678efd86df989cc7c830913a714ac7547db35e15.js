...
var List = React.createClass({displayName: 'List',
  ...
  filterList: function() {
    if(this.state.filterText)
    {
      this.props.shownItems = _.filter(this.props.items, function(i){
        return (i.name.indexOf(this.state.filterText) !== -1 ) || ( i.email.indexOf(this.state.filterText) !== -1  )
     });
    } else
    {
  ...
  handleKeyDown: function(){
    console.log( this.refs.filterSearchBar.getDOMNode());
    this.setState(
      {
        filterText: this.refs.filterSearchBar.getDOMNode()
      }
    );
    this.filterList();
  },
  render: function(){
    ...
    return (
      React.DOM.div( {className:"List"}, 
        SearchBar( {ref:"filterSearchBar", handleKeyDown:this.handleKeyDown} ),
        React.DOM.ul(null,  " ", this.props.shownItems.map(viewForItem))
      )
    )
  }
});
...
var SearchBar = React.createClass({displayName: 'SearchBar',
  render: function(){
    return (
      React.DOM.input( {className:"SearchBar", onKeyDown:this.props.handleKeyDown.bind(null,this), placeholder:"Search..."})
    )
  }
})
