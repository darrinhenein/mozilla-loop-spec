...
var List = React.createClass({displayName: 'List',
  ...
  handleChange: function(){
    var newText = this.refs.filterSearchBar.getDOMNode().value;
    this.setState(
      {
        filterText: newText
      }
  ...
  filterText: function(){
    ...
    if(this.state.filterText !== '')
    {
      ...
      var tempItems = _.filter(this.props.items, function(i){
        return (i.name.toUpperCase().indexOf(filter) !== -1)
      });
      this.setState({
        shownItems: tempItems
      });
    } else
    {
      this.setState({
  ...
  render: function(){
    ...
    var tempItems = this.props.items;

    if(this.state.filterText !== '')
    {
      var filter = this.state.filterText.toUpperCase();
      tempItems = _.filter(this.props.items, function(i){
        return (i.name.toUpperCase().indexOf(filter) !== -1)
      });
    } else
    {
    ...
    return (
      React.DOM.div( {className:"List"}, 
        ...
        React.DOM.p(null,  this.state.filterText ),
        React.DOM.ul( {className:"scrollable"},  " ", tempItems.map(viewForItem))
      )
    )
  }
});
