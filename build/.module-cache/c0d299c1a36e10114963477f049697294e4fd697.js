...
var Profile = React.createClass({displayName: 'Profile',
  ...
  componentWillMount: function(){
    $.ajax({
      ...
      success: function(data){
        this.setState({
          imageUrl: data.image_urls.bigger
        })
      }.bind(this)
    })
  },
  render: function(){
    return (
      React.DOM.div( {className:["Profile", this.props.selected].join(' ')} , 
        React.DOM.div( {className:"avatar"}, React.DOM.img( {src:this.state.imageUrl} )),
...
var List = React.createClass({displayName: 'List',
  ...
  render: function(){
    viewForItem = function(item, index){
      ...
      return Profile( {key:index, user:item, selected:isSelected} )
    }.bind(this);
    ...
    return (
      React.DOM.div( {className:"List"}, 
        ...
        React.DOM.ul( {className:"scrollable"},  " ", shownItems.map(viewForItem))
      )
    )
  }
});
